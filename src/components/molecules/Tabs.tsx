...
export const Tabs: React.FC<TabsProps> = ({ tabs, openedTab = 0 }) => {
  ...
  const stepOfText = t.tabs.stepOf
    .replace("{current}", String(activeTab + 1))
    ...
    <div id="guide" className="w-full max-w-7xl mx-auto py-8">
      ...
      <div className="card p-4 sm:p-6 mb-6">
        ...
        <div
          ...
          role="tablist"
          aria-label={t.tabs.stepByStepGuide}
        >
          {tabs.map((tab, idx) => {
      ...
      <div className="card p-6 sm:p-10 mb-8">
          ...
          <div className="flex flex-wrap items-center gap-3 mb-3">
            ...
            {currentTab.showMap && (
              <span className="px-2.5 py-1 text-xs font-bold bg-slate-100 text-slate-700 border border-slate-200 rounded flex items-center gap-1">
                <MapPin className="w-3.5 h-3.5 text-blue-700" aria-hidden="true" />
                <span>{t.tabs.includesWorkshopMap}</span>
              </span>
            )}
          </div>
          ...
          <div className="mb-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              {currentTab.keyPoints.map((point, index) => (
                <div
                  key={index}
                  className="p-5 rounded-md border border-slate-200 flex flex-col justify-between bg-white"
                >
                  <div>
                  ...
                  {point.linkUrl && (
                    <div className="mt-4 pt-3 border-t border-slate-200">
                      ...
                      >
                        <span>{point.linkLabel || t.tabs.openOfficialPortal}</span>
        ...
        <div className="mt-10 pt-6 border-t border-slate-200 flex items-center justify-between">
          ...
          >
            <span>{activeTab === tabs.length - 1 ? t.tabs.lastStep : t.tabs.nextStep}</span>
